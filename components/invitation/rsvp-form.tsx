...
function validateForm(values: RSVPFormData, messages: ValidationMessages) {
  const errors: Partial<Record<RSVPField, string>> = {};
  if (!values.guestName.trim()) {
    errors.guestName = messages.nameRequired;
  }
  if (values.guestEmail && !/^\S+@\S+\.\S+$/.test(values.guestEmail)) {
    errors.guestEmail = messages.emailInvalid;
  }
  if (!values.guestEmail.trim() && !values.guestPhone.trim()) {
    errors.guestPhone = messages.contactRequired;
  }
  const partySize = Number(values.partySize);
  if (!Number.isInteger(partySize) || partySize < 1 || partySize > 20) {
    errors.partySize = messages.partySize;
  }
...
export function RSVPForm({ invitationSlug }: RSVPFormProps) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setServerError(error instanceof Error ? error.message : t("errors.submit"));
    } finally {
    ...
    <form onSubmit={onSubmit} className="space-y-4 rounded-lg bg-white p-5 text-neutral-900 shadow-xl sm:p-6" noValidate>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="guest-name">{t("name")} <span aria-hidden="true">*</span></Label>
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-neutral-500">{t("contactHint")}</p>
        {errors.guestPhone ? <p className="text-sm text-red-700" role="alert">{errors.guestPhone}</p> : null}
      </div>

      <fieldset className="space-y-2">
        <legend className="text-sm font-medium">{t("attendance")}</legend>
      ...
      <div className="space-y-2">
        <Label htmlFor="dietary-preferences">{t("dietaryPreferences")}</Label>
      ...
      <div className="space-y-2">
        <Label htmlFor="rsvp-message">{t("message")}</Label>
