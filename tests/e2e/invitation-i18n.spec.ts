...
test("renders localized invitation and submits localized RSVP", async ({ page }) => {
  ...
  await expect(page.getByText("December 24, 2026", { exact: false })).toBeVisible();
  ...
  await page.getByLabel("Name", { exact: false }).fill("English Guest");
  await page.getByLabel("Email", { exact: false }).fill(`en-${Date.now()}@carte.test`);
  await page.getByRole("button", { name: "Submit RSVP", exact: true }).click();
  ...
  await expect(page.getByText("2026年12月24日", { exact: false })).toBeVisible();
  await expect(page.locator('meta[name="description"]')).toHaveAttribute("content", "诚邀你参加 Phase 2 RSVP i18n fixture");

  await page.getByLabel("姓名", { exact: false }).fill("中文访客");
  await page.getByLabel("邮箱", { exact: false }).fill(`zh-${Date.now()}@carte.test`);
